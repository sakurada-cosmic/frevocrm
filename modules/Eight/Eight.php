...
class Eight extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_eight';
	var $table_index= 'eightid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_eightcf', 'eightid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_eight', 'vtiger_eightcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_eight'   => 'eightid',
	    'vtiger_eightcf' => 'eightid');
...
/*'Eight No'=> 'eightno',*/
	...
	var $popup_fields = Array ('eightno');

	// For Alphabetical search
	var $def_basicsearch_col = 'eightno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'eightno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('eightno','assigned_user_id');

	var $default_order_by = 'eightno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Eight::checkWebServiceEntry();
			Eight::createUserFieldTable($moduleName);
			Eight::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Eight'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Eight','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
