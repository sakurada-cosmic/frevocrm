...
class Vseven extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_vseven';
	var $table_index= 'vsevenid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_vsevencf', 'vsevenid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_vseven', 'vtiger_vsevencf');
	...
	var $related_tables = Array( 
					'vtiger_vsevencf' => Array('vsevenid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_vseven'   => 'vsevenid',
	    'vtiger_vsevencf' => 'vsevenid');
	...
	var $list_fields = Array (
		...
		'Vseven No' => Array('vseven', 'vsevenno'),
/*'Vseven No'=> Array('vseven', 'vsevenno'),*/
...
/*'Vseven No'=> 'vsevenno',*/
	...
	var $popup_fields = Array ('vsevenno');

	// For Alphabetical search
	var $def_basicsearch_col = 'vsevenno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'vsevenno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('vsevenno','assigned_user_id');

	var $default_order_by = 'vsevenno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Vseven::checkWebServiceEntry();
			Vseven::createUserFieldTable($moduleName);
			Vseven::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Vseven'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Vseven','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
